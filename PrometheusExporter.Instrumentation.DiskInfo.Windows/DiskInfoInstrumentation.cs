...
internal sealed class DiskInfoInstrumentation : IDisposable
{
        ...
        IMetricManager manager)
    {
        ...
        foreach (var disk in disks)
        {
            ...
            if (disk.SmartType == SmartType.Nvme)
            {
                ...
                nvmeDisks.Add(new NvmeDisk(MakeNvmeMetrics(nvmeMetric, smart, environment.Host, disk, drive), smart));
            }
            else if (disk.SmartType == SmartType.Generic)
            {
                var smart = (ISmartGeneric)disk.Smart;
                genericDisks.Add(new GenericDisk(MakeGenericMetrics(genericMetric, smart, environment.Host, disk, drive), smart));
            }
        }
    ...
    private static IMetricSeries[] MakeNvmeMetrics(IMetric metric, ISmartNvme smart, string host, IDiskInfo disk, string drive)
    {
        IMetricSeries Factory(string id) => metric.Create(MakeTags(host, disk.Index, disk.Model, drive, id));
    ...
    private static IMetricSeries?[] MakeGenericMetrics(IMetric metric, ISmartGeneric smart, string host, IDiskInfo disk, string name)
    {
        IMetricSeries Factory(string id) => metric.Create(MakeTags(host, disk.Index, disk.Model, name, id));
